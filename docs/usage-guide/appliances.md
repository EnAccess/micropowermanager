...
1. If the customer is not registered, first use MPM website interface to register a new customer (see [Customers](customers)).
...
3. Go to the "Customer" menu, find the customer to which the created appliance is to be sold, go to "Sold Appliances", click ":heavy_plus_sign:" and assign the mentioned customer the recently created appliance.
   MPM asks the user to select one of the 2 re-payment scheme options:
   ...
   MPM gives as output the instalment amount the customer has to pay.
   ...
   MPM then calculates the number of instalments under which the total appliance cost is to be paid.
