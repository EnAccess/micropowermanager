...
The purpose of the app is to enable SMS-sending from and to the MPM website interface as well as to be able to send SMS to your customer portfolio through the same website.
For this feature to work, the SMS Gateway App must be installed on the applicable mobile phone (following the steps outlined on the "Get Started with MPM").
Click then on the Copy Device Token and share the token with the headquarters staff.
The users with access to the MPM account’s website interface can then insert the device token under the "Settings" page, thereby interlinking this phone with the MPM account.
