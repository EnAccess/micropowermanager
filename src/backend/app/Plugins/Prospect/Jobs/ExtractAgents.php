...
use App\Plugins\Prospect\Services\ProspectAgentTransformer;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Storage;

class ExtractAgents extends AbstractJob {
    /**
    ...
    public function executeJob(): void {
        try {
            Log::info('Starting Prospect agents extraction job...');

            $data = $this->extractDataFromDatabase();

            if ($data === []) {
                Log::warning('No data to extract for Prospect agents.');
            ...
            Log::info('Prospect agents extraction job completed successfully!', [
                'file' => basename($filePath),
                'records_count' => count($data),
            ]);
        } catch (\Exception $e) {
            Log::error('Error during Prospect agents extraction job: '.$e->getMessage(), [
                'error' => $e->getMessage(),
    ...
    private function extractDataFromDatabase(): array {
        try {
            Log::info('Extracting agent data from database...');
            ...
            $agents = $query->get();
            Log::info('Found '.$agents->count().' agents to process');

            $transformer = resolve(ProspectAgentTransformer::class);
            $agentData = [];

            foreach ($agents as $agent) {
                try {
                    $agentData[] = $transformer->transform($agent);
                } catch (\Exception $e) {
                    Log::warning('Failed to transform agent', [
                        'agent_id' => $agent->id,
                        'error' => $e->getMessage(),
                    ]);
                    continue;
            ...
            Log::info('Successfully processed '.count($agentData).' agents');
    ...
    private function writeCsvFile(array $data, string $fileName): string {
        try {
            ...
            $filePath = "prospect/{$companyId}/agents/{$fileName}";
