...
class PushAgents extends AbstractJob {
    /**
    ...
    public function executeJob(): void {
        try {
            ...
            if ($data === []) {
                Log::info('Prospect: no agent data to push');

                return;
            }
            $payload = ['data' => $data];
            $response = resolve(ProspectApiClient::class)->postAgents($payload);

            if ($response->failed()) {
                Log::error('Prospect: push agents failed', ['status' => $response->status(), 'body' => $response->body()]);
                throw new \RuntimeException('Prospect push agents failed');
            }
            Log::info('Prospect: push agents success', ['count' => count($data)]);
            ...
            Log::error('Prospect: push agents error '.$e->getMessage());
            throw $e;
        }
    }
    ...
    private function loadCsvData(): array {
        ...
        Log::info('Loading agent data from: '.basename($filePath));
