...
class ExternalTransactionControllerTest extends TestCase {
    ...
    public function testRegistersTransactionAndDispatchesProcessPaymentJob(): void {
        ...
        $this->assertEquals('USSD', $thirdPartyTransaction->source);
        $this->assertEquals('Payment generated by Vodacom from USSD', $thirdPartyTransaction->description);
    }
    ...
    public function testRejectsMissingSource(): void {
        $token = $this->createApiKey();
        $this->createAppliancePersonWithSerial('SERIAL-006');

        $response = $this->withHeader('Authorization', "Bearer {$token}")->postJson(self::PAY_URL, [
            'serial' => 'SERIAL-006',
            'amount' => 100,
            'external_reference' => 'vodacom-ref-6',
        ]);
