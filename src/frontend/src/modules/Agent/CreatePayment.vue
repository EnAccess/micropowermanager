<template>
    <div>
        <div class="md-layout md-gutter">
            <!-- Payment Form Card -->
            <div class="md-layout-item md-xlarge-size-60 md-large-size-60 md-medium-size-100 md-small-size-100">
                <md-card>
                    <md-card-header>
                        <div class="md-title">{{ $tc("phrases.createPaymentForCustomer") }}</div>
                        <div class="md-subhead">{{ $tc("phrases.generatePaystackPaymentLink") }}</div>
                    </md-card-header>
                    <md-card-content>
                            ...
                            <div class="md-layout md-gutter">
                                <div class="md-layout-item md-size-100">
                                    <md-field>
                                        ...
                                        <md-select v-model="paymentForm.customerId" :disabled="loading"
                                            @md-selected="onCustomerSelected">
                                            <md-option v-for="customer in customers" :key="customer.id"
                                                :value="customer.id">
                            ...
                            <div class="md-layout md-gutter"
                                ...
                                <div class="md-layout-item md-size-100">
                                    <md-field>
                                        ...
                                        <md-select v-model="paymentForm.deviceSerial"
                                            @md-selected="onDeviceSerialSelected" :disabled="loading">
                                            <md-option v-for="device in selectedCustomer.devices"
                                                :key="device.device_serial" :value="device.device_serial"
                            ...
                            <div class="md-layout md-gutter">
                                <div class="md-layout-item md-size-100">
                                    <md-field>
                                        ...
                                        <md-input v-model="paymentForm.amount" type="number" step="0.01" min="0.01"
                            ...
                            <div class="md-layout md-gutter">
                                <div class="md-layout-item md-size-100">
                                    <md-field>
                                        ...
                                        <md-select v-model="paymentForm.currency" :disabled="loading">
                            ...
                            <div class="md-layout md-gutter">
                                <div class="md-layout-item md-size-100">
                                    <md-field>
                                        ...
                                        <md-select v-model="paymentForm.purpose" :disabled="loading">
                                            <md-option value="energy">{{ $tc("words.energy") }} {{ $tc("phrases.topUp")
                                                }}</md-option>
                                            <md-option value="appliance">{{ $tc("words.appliance") }} {{
                                                $tc("words.payment") }}</md-option>
                                            <md-option value="maintenance">{{ $tc("words.maintenance") }} {{
                                                $tc("words.fee") }}</md-option>
                                            <md-option value="other">{{ $tc("words.other") }}</md-option>
                                        </md-select>
                                    </md-field>
                                </div>
                            </div>
                                    ...
                                    <md-card class="commission-preview">
                                        <md-card-content>
                                            <div class="commission-info">
                                                <md-icon class="commission-icon">account_balance_wallet</md-icon>
                                                <div>
                                                    <strong>{{ $tc("phrases.expectedCommission") }}: </strong>
                                                    {{ formatCurrency(expectedCommission, paymentForm.currency) }}
                                                </div>
                                            </div>
                                        </md-card-content>
                                    </md-card>
                                ...
                                <div class="md-layout-item md-size-100">
                                    <div class="form-actions">
                                        <md-button type="submit" class="md-raised md-primary"
                                            ...
                                            <span v-else>{{ $tc("phrases.generatePaymentLink") }}</span>
                                        </md-button>
                                        <md-button type="button" class="md-raised" @click="resetForm"
                                            :disabled="loading">
                                            {{ $tc("words.reset") }}
                                        </md-button>
                                    </div>
            ...
            <div class="md-layout-item md-xlarge-size-40 md-large-size-40 md-medium-size-100 md-small-size-100">
                <div v-if="generatedPayment">
                    <payment-link-card :payment="generatedPayment" :customer="selectedCustomer"
                ...
                <div class="recent-payments" v-if="recentPayments.length > 0">
                    <md-card>
                        <md-card-header>
                            <div class="md-title">{{ $tc("phrases.recentPaymentLinks") }}</div>
                        </md-card-header>
                        <md-card-content>
                            <div class="payment-history-list">
                                <div v-for="payment in recentPayments" :key="payment.id" class="payment-history-item"
                                    @click="selectRecentPayment(payment)">
                                    <div class="payment-info">
                                        <div class="customer-name">{{ payment.customerName }}</div>
                                        <div class="payment-amount">{{ formatCurrency(payment.amount, payment.currency)
                                            }}</div>
    ...
    methods: {
        ...
        onCustomerSelected(customerId) {
            ...
            if (this.selectedCustomer && this.selectedCustomer.devices.length === 1) {
                this.paymentForm.deviceSerial = this.selectedCustomer.devices[0].device_serial
                this.paymentForm.deviceType = this.selectedCustomer.devices[0].device_type
            } else {
        ...
        async createPayment() {
            ...
            try {
                ...
                const response = await this.paystackService.createTransaction(transactionData)

                if (response && response.data) {
                    if (response.data.data?.error) {
                        this.alertNotify("error", `Failed to generate payment link: ${response.data.data.error}`)
                        return
                    }
                    ...
                    this.generatedPayment = {
                        ...
                        customerName: this.selectedCustomer.name + " " + this.selectedCustomer.surname,
