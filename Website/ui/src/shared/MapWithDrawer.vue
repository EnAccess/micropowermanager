    ...
    <div>
        <div style="margin-top: 20px" class="col-md-3">
            <div style="margin-bottom: 10px">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <button
                            class="btn btn-sm"
                            :class="drawCluster ? 'btn-warning' : 'btn-info'"
                            @click="manualDrawwing"
                            v-text="
                                drawCluster ? 'Stop Drawing' : 'Manual Drawing'
                            "
                        ></button>
                    </div>
                    <div class="col-sm-12" style="margin-top: 5px">
                        <button
                            @click="getClusterGeoData"
            ...
            <ul class="list-group" v-if="!drawCluster">
                ...
                <li
                    v-if="drawnPolygons.initialized"
                    @click="selectManual"
                    class="list-group-item"
            ...
            <ul class="list-group" v-else>
                <li
                    class="list-group-item"
                    v-for="(coordinate, index) in polygonCoordinates"
                    :key="index"
                >
                    <i
    ...
    methods: {
        ...
        selectManual() {
            ...
            if (!this.manualLayer.getLayers().length) {
                this.drawnPolygons.polygon = new L.geoJSON(
                    this.drawnPolygons.geoJson,
                )
        ...
        async manualDrawwing() {
            ...
            if (this.drawCluster) {
                ...
                //convert poly line to polygon+
                this.drawnPolygons.polygon = new L.Polygon(
                    this.polygon.getLatLngs(),
                )
                this.drawnPolygons.polygon.addTo(this.manualLayer)

                this.drawnPolygons.geoJson =
                    this.drawnPolygons.polygon.toGeoJSON()
        ...
        initMap() {
            ...
            this.tileLayer = L.tileLayer(
                ...
                'http://{s}.tile.osm.org/{z}/{x}/{y}.png',
                {
                    maxZoom: 18,
                    attribution:
                        ' <span style="cursor:pointer">&copy; MpManager</span>',
                },
            )
            ...
            this.map.on('click', function (e) {
                if (parent.drawCluster === true) {
                    ...
                    if (parent.polygon === null) {
                        parent.polygon = L.polyline([[lat, lng]]).addTo(
                            parent.map,
                        )
                    }
