    ...
    <md-table>
      ...
      <template v-for="(ticket,index) in ticketList">
        <md-table-row @click="openTicket(index)" :key="'tic'+index">
          <md-table-cell>
            <md-icon>{{ showTicket === index ? 'keyboard_arrow_down' : 'keyboard_arrow_right' }}</md-icon>
          </md-table-cell>
          <md-table-cell>{{ ticket.title }}</md-table-cell>
          <md-table-cell v-if="ticket.category">{{ ticket.category }}</md-table-cell>
          ...
          <md-table-cell :colspan="tableHeads.length + 1">
            ...
            <div class="ticket-desc">
              ...
              <div class="md-layout-item md-size-100 t-text-area">
                ...
                <span
                    v-if="ticket.owner !== undefined && ticket.owner !== null">{{ ticket.owner.name }} {{ ticket.owner.surname }} :</span>
              ...
              <div class="md-layout-item md-size-95 new-comment-area" v-if="showComments">
                ...
                <md-field>
                  <label>{{ $tc('phrases.newComment') }}</label>
