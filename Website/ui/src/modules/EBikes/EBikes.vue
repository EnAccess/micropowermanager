<template>
    <div>
        <AddEBikeModal :showAddEBike="showAddEBike"
                       @hideAddEBike="() => { showAddEBike = false}"
                       @created="updateList"/>
        <EBikeDetailModal :e-bike="eBikeService.eBike" :showEBikeDetail="showEBikeDetail"
                          @hideEBikeDetail="() => { showEBikeDetail = false }"/>
        <widget
            :id="'eBike-list'"
            ...
            :route_name="'/e-bikes'"
            color="green"
            @widgetAction="() => { showAddEBike = true }"
        >
            <md-table md-card style="margin-left: 0">
                <md-table-row>
                    <md-table-head>{{ $tc('phrases.serialNumber') }}</md-table-head>
                ...
                >
                    ...
                    <md-table-cell> {{ timeForTimeZone(eBike.updatedAt) }}</md-table-cell>
                </md-table-row>
            </md-table>
        </widget>
    </div>
...
export default {
    ...
    methods: {
        reloadList (subscriber, data) {
            ...
            EventBus.$emit('widgetContentLoaded', this.subscriber, this.eBikeService.list.length)
        },
        updateList (eBike) {
