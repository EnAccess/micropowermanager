    ...
    <div>
        ...
        >
            ...
            <md-table md-sort="id" md-sort-order="asc">
                ...
                <md-table-row
                    v-for="(item, index) in agentCommissionService.list"
                    :key="index"
                >
                    ...
                    <md-table-cell md-label="Appliance Commission">
                        <div v-if="editCommission === item.id">
                            <md-field
                                :class="{
                                    'md-invalid': errors.has(
                                        $tc('phrases.applianceCommission'),
                                    ),
                                }"
                            >
                                <label>
                                ...
                                <md-input
                                    :name="$tc('phrases.applianceCommission')"
                                    id="applianceCommission"
                    ...
                    <md-table-cell md-label="Risk Balance">
                        <div v-if="editCommission === item.id">
                            ...
                            >
                                ...
                                <span class="md-error">
                                    {{
                                        errors.first($tc('phrases.riskBalance'))
                                    }}
                                </span>
                            </md-field>
                        </div>
...
export default {
    ...
    methods: {
        ...
        async getAgentCommissions() {
            try {
                ...
                EventBus.$emit(
                    'widgetContentLoaded',
                    this.subscriber,
                    this.agentCommissionService.list.length,
                )
            } catch (e) {
                this.loading = false
        ...
        async updateCommission(commission) {
            try {
                this.loading = true
                await this.agentCommissionService.updateAgentCommission(
                    commission,
                )
                this.alertNotify(
                    'success',
                    this.$tc('phrases.agentCommissionUpdated'),
                )
                await this.getAgentCommissions()
                this.loading = false
            } catch (e) {
        ...
        async deleteCommission(commissionId) {
            try {
                this.loading = true
                await this.agentCommissionService.deleteAgentCommission(
                    commissionId,
                )
                this.alertNotify(
                    'success',
                    this.$tc('phrases.agentCommissionDeleted'),
                )
                await this.getAgentCommissions()
                this.loading = false
            } catch (e) {
