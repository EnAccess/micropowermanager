<template>
    <div>
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <md-table-cell
                        :md-label="$tc('words.phone')"
                        md-sort-by="phone"
                    >
                        {{ item.phone }}
                    </md-table-cell>
                    <md-table-cell
                        :md-label="$tc('words.primary')"
                        md-sort-by="primary"
                    >
                        <input
                            type="checkbox"
        ...
        <md-dialog
            class="address-edit-container md-size-100"
            :md-active.sync="modalVisibility"
        >
            <md-dialog-title v-if="editFlag">
                {{ $tc('phrases.updateAddress') }}
            </md-dialog-title>
            <md-dialog-title v-if="!editFlag">
                {{ $tc('phrases.newAddress') }}
            </md-dialog-title>
                        ...
                        <md-field name="city">
                            ...
                            >
                                <md-option
                                    ...
                                    disabled
                                    v-if="
                                        !editFlag || newAddress.city_id === null
                                    "
                                >
                                    {{ $tc('words.city') }}
                                </md-option>
            ...
            if (this.validateNewAddress()) {
                ...
                if (this.editFlag) {
                    this.addresses
                        ...
                        .then((response) => {
                            this.addresses.list = this.addresses.list.map(
                                function (item) {
                                    ...
                                    if (item.id === response.data.data.id) {
                                        ...
                                        return updatedAddress.fromJson(
                                            response.data.data,
                                        )
                                    }
                                    return item
            ...
            if (!('city_id' in this.newAddress) || !this.newAddress.city_id) {
                ...
                if (!this.newAddress.phone.startsWith('+')) {
                    if (this.newAddress.phone.startsWith('00')) {
                        this.newAddress.phone = this.newAddress.phone.replace(
                            '00',
                            '+',
                        )
                    } else {
