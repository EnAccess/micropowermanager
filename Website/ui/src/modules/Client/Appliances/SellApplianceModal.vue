                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <md-input
                                            ...
                                            min="0"
                                            v-model="
                                                applianceService.appliance
                                                    .downPayment
                                            "
                                            v-validate="
                                                'required|min_value:0|decimal'
                                            "
                                            @keyup="checkDownPayment"
                                        />
                                        <span class="md-error">
    ...
    methods: {
            ...
            this.$swal({
                ...
                if (result.value) {
                    try {
                        ...
                        const soldAppliance =
                            await this.assetPersonService.sellAppliance(
                                soldApplianceParams,
                            )
                        this.alertNotify(
                            'success',
        ...
        async getGeoPointsForAppliance() {
            ...
            try {
                ...
                const result = results.reduce((prev, current) => {
                    if (address.street) {
                        return prev.confidence > current.confidence
                            ? prev
                            : current
                    }
        ...
        calculateRateCountsOnRateCostChange() {
            const remainingCost =
                parseFloat(this.applianceService.appliance.cost) -
                parseFloat(this.applianceService.appliance.downPayment)
            const installmentCost = Number(
                this.applianceService.appliance.rateCost,
            )
