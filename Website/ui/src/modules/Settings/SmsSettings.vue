        ...
        <md-tabs>
            ...
            >
                <div
                    v-for="(
                        smsBody, index
                    ) in smsBodiesService.confirmationList"
                    :key="index"
                >
                    <sms-body
                        ref="smsBody_confirmation_ref"
            ...
            <md-tab
                ...
                id="tab-resend-information"
                md-label="Resend Last Transaction"
            >
                        ...
                        <md-field
                            :class="{
                                'md-invalid': errors.has(
                                    'resend_information_key',
                                ),
                            }"
                        >
                            <label for="resend_information_key">
                                Resend Last Transaction Information Key
                            </label>
                            <md-input
                                v-model="
                                    smsResendInformationKeyService
                                        .smsResendInformationKey.key
                                "
                            ...
                            ></md-input>
                            <span class="md-error">
                                {{ errors.first('resend_information_key') }}
                            </span>
                        </md-field>
                    </div>
                    <div class="md-layout-item md-size-100">
                        <div
                            v-for="(
                                smsBody, index
                            ) in smsBodiesService.resendInformationList"
                            :key="index"
                        >
                            <sms-body
                                ref="smsBody_resend_ref"
                                :sms-variable-default-values="
                                    smsVariableDefaultValueService.list
                                "
                                :sms-body="smsBody"
...
export default {
    ...
    data() {
        return {
            ...
            smsResendInformationKeyService:
                new SmsResendInformationKeyService(),
            smsAndroidSettingsService: new SmsAndroidSettingService(),
            smsVariableDefaultValueService:
                new SmsVariableDefaultValueService(),
            isValid: false,
        }
    },
    created() {
    ...
    mounted() {
        ...
        EventBus.$on(
            'smsAndroidSettingSaved',
            this.saveAdditionalAndroidSetting,
        )
    },
    methods: {
        ...
        async updateSmsBodies() {
            if (this.tab === 'confirmation') {
                ...
                if (
                    !this.smsBodiesService.confirmationList.filter(
                        (x) => !x.validation,
                    ).length
                ) {
                    try {
                ...
                await this.validateSmsBodies(refs)
                if (
                    !this.smsBodiesService.reminderList.filter(
                        (x) => !x.validation,
                    ).length
                ) {
                    try {
                ...
                await this.validateSmsBodies(refs)
                if (
                    !this.smsBodiesService.resendInformationList.filter(
                        (x) => !x.validation,
                    ).length
                ) {
                    try {
