...
export default {
    ...
    mounted() {
        ...
        EventBus.$on('listChanged', () => {
            this.resetKey += 1
            EventBus.$emit('widgetContentLoaded', this.subscriber.opened, this.ticketService.openedList?.length ?? 0)
            EventBus.$emit('widgetContentLoaded', this.subscriber.closed, this.ticketService.closedList?.length ?? 0)
